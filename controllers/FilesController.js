...
const postUpload = async (req, res) => {
    ...
    const object_id = new ObjectId(await redisClient.get(key))
    const getUser = await usersCollection.findOne({ '_id': object_id })
    if (!getUser) {
    ...
    if (type === 'folder'){
        ...
        const returned = await filesCollection.findOne(
            { 'userId': getUser['_id'], 'name': name }, { 'localPath' : 0}
        )
...
const getShow = async (req, res) => {
    ...
    const o_id = new ObjectId(fileId['id'])
    const getUserFromDB = await filesCollection.findOne({ '_id': o_id })
    if (!getUserFromDB) {
...
const putUnpublish = async (req, res) => {
    ...
    const object_id = new ObjectId(getUserFromRedis)
